...
const Header = () => {
    ...
    return (
        ...
        <div className={`w-full flex items-center lg:pt-[61px] sm:pt-[40px] pt-[25px] lg:px-0 sm:px-2 px-2 fixed ${scrolled ? "bg-white !py-[20px] dark:bg-darkbg" : "bg-transparent"}  top-0 z-20`}>
            ...
            <div className="w-full flex justify-between items-center max-w-[1420px] mx-auto">
                ...
                <nav className="lg:flex hidden items-center xl:gap-[24px] lg:gap-[16px]">
                    ...
                    <Button className="xl:w-[188px] cursor-pointer w-[170px] xl:h-[52px] h-[40px] font-[400] font-poppins xl:text-[21px] lg:text-[18px] text-[16px] tracking-[0.6px] leading-[100%]" variant="primary">
                        Download CV
                    </Button>
                </nav>
                ...
                <div className="lg:hidden">
                    <button className="w-[40px] h-[40px] p-0 dark:text-white" onClick={handleMenuClick}>
                    ...
                    </button>
                    {
                        isMenuOpen && (
                            <div
                                className={`absolute max-w-[300px] h-screen
    ${isMenuOpen ? "translate-x-0" : "translate-x-full"}  
    transition duration-500 ease-in-out
    p-[32px] w-full top-0 right-0  
    bg-primary shadow-lg z-50 opacity-95`}
                            >
                                <div className="flex justify-between mb-4">
                                    <DarkModeToggle />
                                    <button onClick={handleMenuClick} className="text-2xl cursor-pointer text-white">X</button>
                                </div>
                                <ul className="flex border-t border-border pt-8  flex-col gap-[12px] text-secondary mx-auto justify-center text-center">
                                    {navItems.map((item) => (
                                        ...
                                        >
                                            <a href={item.href} className="hover:text-secondary text-white cursor-pointer">
                                                {item.name}
                                            </a>
                                        </li>
                                    ))}
